	...
	load_promise := Promise.all([
		...
		new Promise (resolve, reject) !->
			script	= document.createElement("script")
				..async		= true
				..src		= '/modules/TinyMCE/assets/js/tinymce.min.js'
				..onload	= resolve
				..onerror	= reject
		...
		tinymce
			...
			..baseURL								= '/modules/TinyMCE/assets/js'
			..editor_config_full					= {
...
window.TinyMCE_Polymer_editor_behavior =
	...
	_editor_change_callback : (editor) !->
		...
		change_timeout	= setTimeout (!~>
			...
			@value	= @_tinymce_editor.getContent()
			event	= document.createEvent('Event')
			event.initEvent('change', false, true)
			@_tinymce_editor.getElement().dispatchEvent(event)
		), 100
	_value_changed : !->
