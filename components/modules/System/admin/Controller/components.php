		...
		if (
			...
			switch ($rc[2]) {
				case 'install':
					if ($rc[3] == 'upload' && isset($_FILES['upload_module']) && $_FILES['upload_module']['tmp_name']) {
						...
						if (isset($Config->components['modules'][$module_name])) {
							...
							$check_dependencies = static::check_dependencies($module_name, 'module', $tmp_dir, 'update');
					...
					$check_dependencies = static::check_dependencies($rc[3], 'module', null, 'install');
					if (!$check_dependencies && $Config->core['simple_admin_mode']) {
						break;
					}
				...
				case 'uninstall':
					...
					$check_dependencies = static::check_backward_dependencies($rc[3], 'module', 'uninstall');
					if (!$check_dependencies && $Config->core['simple_admin_mode']) {
						break;
					}
					$a->cancel_button_back = true;
				...
				case 'enable':
					$show_modules       = false;
					$check_dependencies = static::check_dependencies($rc[3], 'module', null, 'enable');
					if (!$check_dependencies && $Config->core['simple_admin_mode']) {
						break;
					}
					Event::instance()->fire(
				...
				case 'disable':
					$show_modules       = false;
					$check_dependencies = static::check_backward_dependencies($rc[3], 'module', 'disable');
					if (!$check_dependencies && $Config->core['simple_admin_mode']) {
						break;
					}
					Event::instance()->fire(
			...
			switch ($rc[2]) {
				case 'enable':
					if ($rc[3] == 'upload' && isset($_FILES['upload_plugin']) && $_FILES['upload_plugin']['tmp_name']) {
						...
						if (in_array($plugin, $plugins)) {
							...
							$check_dependencies = static::check_dependencies($plugin, 'plugin', $tmp_dir);
							if (!$check_dependencies && $Config->core['simple_admin_mode']) {
								break;
							}
							$rc[2] = 'update';
					...
					if (!in_array($rc[3], $Config->components['plugins']) && in_array($rc[3], $plugins)) {
						...
						$check_dependencies = static::check_dependencies($rc[3], 'plugin');
						if (!$check_dependencies && $Config->core['simple_admin_mode']) {
							break;
						}
				...
				case 'disable':
					if (in_array($rc[3], $Config->components['plugins'])) {
						...
						$check_dependencies = static::check_backward_dependencies($rc[3], 'plugin');
						if (!$check_dependencies && $Config->core['simple_admin_mode']) {
							break;
						}
						$a->cancel_button_back = true;
