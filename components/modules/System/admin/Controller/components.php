...
trait components {
		...
		if (isset($rc[2])) {
			switch ($rc[2]) {
				...
				case 'delete':
					...
					$Page->title($L->deletion_of_block(self::get_block_title($rc[3])));
					$a->content(
						h::{'h2.cs-center'}(
							$L->sure_to_delete_block(self::get_block_title($rc[3])).
							h::{'input[type=hidden]'}(
								[
									'name'  => 'mode',
									'value' => 'delete'
				...
				case 'edit':
					...
					$Page->title($L->editing_a_block(self::get_block_title($rc[3])));
					$a->content(
						h::{'h2.cs-center'}(
							$L->editing_a_block(self::get_block_title($rc[3]))
						).
						h::{'cs-table[center][right-left] cs-table-row| cs-table-cell'}(
							[
								h::info('block_title'),
								h::input(
									[
										'name'  => 'block[title]',
										'value' => self::get_block_title($rc[3])
									]
								)
							],
							[
				...
				WHERE `permission` = '%s'",
					...
					unset($user, $value);
					$Page->title($L->permissions_for_block(self::get_block_title($rc[3])));
					$a->content(
						h::{'h2.cs-center'}(
							$L->permissions_for_block(self::get_block_title($rc[3]))
						).
						h::{'ul.cs-tabs li'}(
							$L->groups,
							$L->users
			...
			if (!empty($Config->components['blocks'])) {
				foreach ($Config->components['blocks'] as $id => $block) {
					$blocks_array[$block['position']] .= h::li(
						h::{'div.cs-blocks-items-title'}("#$block[index] ".self::get_block_title($id)).
						h::a(
							[
								h::{'div icon'}('pencil'),
								[
		...
		if (
			...
			switch ($rc[2]) {
				case 'install':
					if ($rc[3] == 'upload' && isset($_FILES['upload_module']) && $_FILES['upload_module']['tmp_name']) {
						...
						if (isset($Config->components['modules'][$module_name])) {
							...
							$check_dependencies = self::check_dependencies($module_name, 'module', $tmp_dir, 'update');
					...
					$check_dependencies = self::check_dependencies($rc[3], 'module', null, 'install');
					if (!$check_dependencies && $Config->core['simple_admin_mode']) {
						break;
					}
				...
				case 'uninstall':
					...
					$check_dependencies = self::check_backward_dependencies($rc[3], 'module', 'uninstall');
					if (!$check_dependencies && $Config->core['simple_admin_mode']) {
						break;
					}
					$a->cancel_button_back = true;
				...
				case 'enable':
					$show_modules       = false;
					$check_dependencies = self::check_dependencies($rc[3], 'module', null, 'enable');
					if (!$check_dependencies && $Config->core['simple_admin_mode']) {
						break;
					}
					Event::instance()->fire(
				...
				case 'disable':
					$show_modules       = false;
					$check_dependencies = self::check_backward_dependencies($rc[3], 'module', 'disable');
					if (!$check_dependencies && $Config->core['simple_admin_mode']) {
						break;
					}
					Event::instance()->fire(
	 ...
	 * @return bool
	 */
	static private function check_dependencies ($name, $type, $dir = null, $mode = 'enable') {
		if (!$dir) {
			switch ($type) {
				case 'module':
					$dir = MODULES."/$name";
	 ...
	 * @return bool
	 */
	static private function check_backward_dependencies ($name, $type = 'module', $mode = 'disable') {
		switch ($type) {
			case 'module':
				$dir = MODULES."/$name";
				break;
	 ...
	 * @return array
	 */
	static private function dep_normal ($dependence_structure) {
		$return = [];
			...
			switch ($rc[2]) {
				case 'enable':
					if ($rc[3] == 'upload' && isset($_FILES['upload_plugin']) && $_FILES['upload_plugin']['tmp_name']) {
						...
						if (in_array($plugin, $plugins)) {
							...
							$check_dependencies = self::check_dependencies($plugin, 'plugin', $tmp_dir);
							if (!$check_dependencies && $Config->core['simple_admin_mode']) {
								break;
							}
							$rc[2] = 'update';
					...
					if (!in_array($rc[3], $Config->components['plugins']) && in_array($rc[3], $plugins)) {
						...
						$check_dependencies = self::check_dependencies($rc[3], 'plugin');
						if (!$check_dependencies && $Config->core['simple_admin_mode']) {
							break;
						}
				...
				case 'disable':
					if (in_array($rc[3], $Config->components['plugins'])) {
						...
						$check_dependencies = self::check_backward_dependencies($rc[3], 'plugin');
						if (!$check_dependencies && $Config->core['simple_admin_mode']) {
							break;
						}
						$a->cancel_button_back = true;
